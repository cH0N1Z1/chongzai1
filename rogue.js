...
function rogueSaveMeta() {
  try { localStorage.setItem('douro2RogueMeta', JSON.stringify(ROGUE.meta)); } catch (e) {}
}
...
function rogueStartRun() {
  ...
  try {
    ...
    if (raw) {
      ...
      player.soulPower = sp;
      player.maxHp = 40 + sp * 8;
      player.hp = player.maxHp;
      player.atk = 8 + Math.floor(sp * 0.8);
      ...
      const rings = c.rings || [];
      const skillList = [];
      const tierOrder = ['十年', '百年', '千年', '万年', '十万年'];
      for (const tier of tierOrder) {
        if (rings.some(r => (r.name || r || '').includes(tier))) {
          const sk = ROGUE_DATA.skills[tier] || [];
          sk.forEach(s => skillList.push({ ...s }));
        }
      }
...
function rogueCreateEvent(type) {
  ...
  if (type === 'battle' || type === 'elite') {
    ...
    return {
      ...
      isElite: type === 'elite',
      name: m[2],
      hp: Math.round(m[4] * scale),
      maxHp: Math.round(m[4] * scale),
      atk: Math.round(m[3] * scale),
      gold: [m[5][0], m[5][1]],
      material: m[6],
      element: m[7],
      turn: 1,
  ...
  if (type === 'treasure') {
    const g = rogueRoll(5 + d * 3, 12 + d * 6);
    return { kind: 'treasure', gold: g };
  }
  if (type === 'spring') {
    return { kind: 'spring' };
  }
...
function rogueAttack() {
  ...
  const p = s.player;
  const m = s.current;
  const base = p.atk + rogueRoll(-2, 3);
  const dmg = Math.max(1, Math.round(base * rogueElementMult(p.element, m.element)));
  m.hp -= dmg;
...
function rogueUseSkill(idx) {
  ...
  p.soulPower -= sk.cost;
  const m = s.current;
  const base = sk.dmg + rogueRoll(-3, 5);
  const dmg = Math.max(1, Math.round(base * rogueElementMult(p.element, m.element)));
  m.hp -= dmg;
...
function rogueUsePotion() {
  ...
  s.log.unshift(`你喝下药水，恢复 ${heal} 点生命。`);
  rogueEnemyTurn();
}
...
function rogueEnemyTurn() {
  ...
  const p = s.player;
  if (m.hp <= 0) return;
  const base = m.atk + rogueRoll(-2, 3);
  const dmg = Math.max(1, Math.round(base * rogueElementMult(m.element, p.element)));
  p.hp -= dmg;
...
function rogueVictory() {
  const s = ROGUE.state;
  const m = s.current;
  const g = rogueRoll(m.gold[0], m.gold[1]);
  s.loot.gold += g;
...
function rogueOpenTreasure() {
  ...
  const c = s.current;
  s.loot.gold += c.gold;
  s.log.unshift(`打开宝箱，获得 ${c.gold} 金币。`);
  s.phase = 'reward';
  s.current = null;
  rogueRender();
}
...
function rogueRetreat() {
  const s = ROGUE.state;
  ROGUE.running = false;
  const earned = s.loot.gold;
  ROGUE.meta.gold += earned;
...
function rogueRender() {
  ...
  if (s.phase === 'dead') {
    body = `
      <div class="rogue-dead">
        <h3>☠ 你倒下了</h3>
        <p>本局收获（${s.loot.gold} 金币、${s.loot.materials.length} 件材料）全部丢失。</p>
        <button class="rogue-btn-primary" onclick="rogueShowLobby()">回到大厅</button>
      </div>`;
    ...
    body = `
      <div class="rogue-battle">
        <div class="rogue-enemy">
          <div class="rogue-enemy-name">${m.isElite ? '★ ' : ''}${m.name} <span class="rogue-el">${m.element}</span></div>
    ...
    body = `
      <div class="rogue-event">
        <h3>💰 发现宝箱</h3>
        <p>一个古老的魂导宝箱静静躺在树根下。</p>
        <button class="rogue-btn-primary" onclick="rogueOpenTreasure()">打开</button>
      </div>`;
  } else if (s.phase === 'spring') {
    body = `
      <div class="rogue-event">
        <h3>💧 生命泉</h3>
        <p>清澈的泉水泛着微光，喝一口感觉浑身舒畅。</p>
        <button class="rogue-btn-primary" onclick="rogueDrinkSpring()">饮下</button>
      </div>`;
  } else if (s.phase === 'shrine') {
    body = `
      <div class="rogue-event">
        <h3>⛩ ${s.current.name}</h3>
        <p>一座古老神龛，散发着温和的力量。</p>
        <button class="rogue-btn-primary" onclick="rogueUseShrine()">祈祷</button>
      </div>`;
  } else if (s.phase === 'retreat') {
    body = `
      <div class="rogue-event">
        <h3>成功脱离</h3>
        <p>你暂时摆脱了危险，可以选择继续深入或撤退。</p>
        <div class="rogue-actions">
    ...
    body = `
      <div class="rogue-event">
        <h3>✓ 收获</h3>
        <p>当前：第 ${s.depth} 层 · 金币 ${s.loot.gold} · 材料 ${s.loot.materials.length} 件</p>
        <div class="rogue-actions">
          <button class="rogue-btn-primary" onclick="rogueAdvance()">继续深入第 ${s.depth + 1} 层</button>
          <button class="rogue-btn-ghost" onclick="rogueRetreat()">撤退，保留收获</button>
        </div>
  ...
  el.innerHTML = `
    <div class="rogue-header">
      <div class="rogue-title">第 ${s.depth} 层</div>
      <button class="rogue-btn-ghost rogue-exit-btn" onclick="rogueExit()">×</button>
    </div>
    <div class="rogue-status">
      <div class="rogue-bar rogue-bar-hp">
        <div class="rogue-bar-fill rogue-hp-player" style="width:${hpPct}%"></div>
        <span class="rogue-bar-text">❤ ${p.hp} / ${p.maxHp}</span>
      </div>
      <div class="rogue-stats">
        <span>⚔ 攻击 ${p.atk}</span>
        <span>⚡ 魂力 ${p.soulPower}</span>
        <span>💰 ${s.loot.gold}</span>
        <span>🧪 ${p.potions}</span>
      </div>
    </div>
    ${body}
